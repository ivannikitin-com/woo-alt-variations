...
?>
<?php echo do_shortcode( '[all_var_reviews]' ); ?>

<div id="reviews" class="woocommerce-Reviews">
	<div id="comments">
		<h2 class="woocommerce-Reviews-title"><?php
			/*if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = $product->get_review_count() ) ) {*/
			if ( get_option( 'woocommerce_enable_review_rating' ) === 'yes' && ( $count = count($aggr_comments) ) ) {
				/* translators: %s: Number of reviews. */
				printf(_n( '%s review', '%s reviews', count($aggr_comments), 'woocommerce' ),
			count($aggr_comments));
			} else {
		...
		<?php if ( count($aggr_comments) ) : ?>
			<ol class="commentlist">
				<?php wp_list_comments( apply_filters( 'woocommerce_product_review_list_args', array( 'callback' => 'woocommerce_comments' ) ), $aggr_comments ); ?>
			</ol>

			<?php if ( get_comment_pages_count($aggr_comments) > 1 && get_option( 'page_comments' ) ) :
				echo '<nav class="woocommerce-pagination">';
				paginate_comments_links( apply_filters( 'woocommerce_comment_pagination_args', array(
					'prev_text' => '&larr;',
					'next_text' => '&rarr;',
					'type'      => 'list',
					'total'   => get_comment_pages_count($aggr_comments),
